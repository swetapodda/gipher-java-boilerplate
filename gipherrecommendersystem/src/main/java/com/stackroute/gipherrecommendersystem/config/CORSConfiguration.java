	...
	public WebMvcConfigurer corsConfigurer() {
		return new WebMvcConfigurer() {
			...
			public void addCorsMappings(CorsRegistry registry) {
				registry.addMapping("/api/v1/gipherrecomendersystem").allowedOrigins("*").allowedMethods("GET");// From all host and only with /api/v1/accountmanager based Requests.
				registry.addMapping("/**").allowedOrigins("*").allowedMethods("GET"); //Note; If we not given allowed Mehtods, post/get works But DELETE blocked by CORS
			}
		};
	}
}

...
/**
 * Swagger API Documentation Configuraiton. Uses Docket and API Generation
 * 
 * L.GANESH
 */
...
public class SwaggerConfiguration {
    ...
    public Docket productApi() {
		 ...
		 return new Docket(DocumentationType.SWAGGER_2).groupName("Keep Note")
		  .apiInfo(apiInfo())
		  .select()
		  .paths(postPaths())
		  .build()
	...
	private Predicate<String> postPaths() {
    	return Predicates.or(PathSelectors.regex("/api/v1/giphermanager.*"));//return or(regex("/api/v1.*"));
	}

	private ApiInfo apiInfo() {
	        ApiInfo apiInfo=new ApiInfoBuilder().title("Gipher Manager Service")
				.description("Gipher Manager Service API for Developers")
